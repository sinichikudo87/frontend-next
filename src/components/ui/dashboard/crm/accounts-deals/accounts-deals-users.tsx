...
interface AccountsDealsUsersProps {
  initialData: AccountDeal[];
}

export default function AccountsDealsUsers({ initialData }: AccountsDealsUsersProps) {
  const [search, setSearch] = useState("");
  const [openRow, setOpenRow] = useState<string | null>("DL-2601-00004");
  
  const [dataDeals] = useState<AccountDeal[]>(initialData);
      ...
      <div className="p-4 md:p-6 text-white space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-bold tracking-wide">Accounts Deals</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-white/50">Closed Deals</p>
                <h2 className="text-3xl font-bold mt-2 text-emerald-400">84</h2>
              </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-white/50">Revenue Pipeline</p>
                <h2 className="text-3xl font-bold mt-2 text-blue-400">Rp 4.2B</h2>
              </div>
                      ...
                      <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-white/60">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                PIC : <span className="text-white/80 font-medium">{item.pic}</span>
                              </div>
                              <div className="flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-3 justify-between xl:justify-end">
                          <div className="px-5 py-3 rounded-2xl bg-gradient-to-br from-yellow-500/20 to-orange-500/10 border border-yellow-500/20">
                            <div className="text-xs text-white/50 mb-1">Deal Value</div>
                            <div className="font-bold text-lg text-yellow-400">{formatCurrency(item.total)}</div>
                          </div>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="text-white/50 border-b border-white/10">
                                <th className="text-left pb-3 font-medium">Vehicle Category</th>
                                <th className="text-center pb-3 font-medium">Qty</th>
                                <th className="text-center pb-3 font-medium">Price</th>
                                <th className="text-center pb-3 font-medium">Subtotal</th>
                                <th className="text-center pb-3 font-medium">Status</th>
                              </tr>
                            </thead>
                            <tbody>
                              {item.details.map((detail, dIdx) => (
                                <tr key={dIdx} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                                  <td className="py-4 font-medium text-white/80 uppercase">{detail.kategori}</td>
                                  <td className="py-4 text-center text-white/70">{detail.qty}</td>
                                  <td className="py-4 text-center text-white/70">{formatCurrency(detail.harga)}</td>
                                  <td className="py-4 text-center font-semibold text-yellow-400">{formatCurrency(detail.subtotal)}</td>
                                  <td className="py-4 text-center">
                                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10">
                                      <Clock3 className="w-3 h-3 text-blue-400" />
                                      <span className="text-xs text-white/70">{detail.status}</span>
                                    </div>
                                  </td>
                                </tr>
                              ))}
