...
export default function ApprovalsPurchasing() {
  ...
  const [berkasList, setBerkasList] = useState<BerkasItem[]>([
    {
      id: 1,
      namaDokumen: "Divisi Purchasing - Form Approval",
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/public/v1/approvals/approvals-form/${encryptedId}`
        );
        ...
        Swal.fire({
          ...
          text: "Data penawaran tidak ditemukan / token invalid",
          background: "#0f172a",
          color: "#fff",
  ...
  if (!approvalDetail) {
    return (
      <div className="min-h-screen flex flex-col gap-4 items-center justify-center text-white bg-slate-950 relative overflow-hidden px-4">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-60 h-60 bg-fuchsia-500/15 rounded-full blur-[100px]" />
        <div className="relative flex items-center justify-center h-14 w-14">
          <div className="animate-ping absolute inline-flex h-full w-full rounded-full bg-fuchsia-400 opacity-75"></div>
          <div className="relative inline-flex rounded-full h-10 w-10 bg-gradient-to-tr from-fuchsia-600 to-blue-500 shadow-lg"></div>
        </div>
  ...
  const handleSubmitSelesai = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        approval_role: "purchasing" as const,
        approver_id: 1,
    ...
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 p-3 sm:p-6 md:p-8 relative overflow-x-hidden selection:bg-fuchsia-500/30">
      ...
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 relative z-10">
        ...
        <div className="flex justify-center sm:justify-start">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r from-fuchsia-500/10 to-blue-500/10 border border-fuchsia-500/20 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-fuchsia-300 backdrop-blur-md">
        ...
        <div className="rounded-2xl sm:rounded-3xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-xl p-4 sm:p-6 md:p-8 shadow-xl space-y-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/[0.06] pb-4 sm:pb-5">
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] px-3 py-1.5 sm:py-2 rounded-xl self-start sm:self-center">
              ...
              <span className="font-mono text-xs sm:text-sm font-bold text-blue-400">{approvalDetail.kode}</span>
            </div>
          </div>

          {/* METADATA GRID (2 Kolom di Mobile, 4 Kolom di Desktop) */}
          ...
          <div className="flex justify-between sm:justify-end items-center bg-gradient-to-r from-white/[0.01] to-white/[0.03] p-3 sm:p-4 rounded-xl border border-white/[0.04]">
            <span className="text-xs text-slate-400 font-medium uppercase tracking-wider sm:hidden">Total</span>
        ...
        <form onSubmit={handleSubmitSelesai} className="space-y-3 sm:space-y-4">
          {berkasList.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border transition-all duration-300 backdrop-blur-md shadow-md ${isApproved
              ...
              >
                ...
                <div className="flex items-start gap-2.5 text-slate-200">
                  ...
                  <div className="w-full">
                    <span className="text-[10px] text-slate-400 block font-medium uppercase tracking-wider">Dokumen Verifikasi</span>
                    <p className="font-bold text-xs sm:text-sm tracking-wide text-white">{item.namaDokumen}</p>
                  </div>
                </div>

                {/* Tombol Keputusan (Full width stacked di HP kecil, sejajar di tablet+) */}
                <div className="grid grid-cols-2 gap-2.5 pt-1">
                  <button
                    type="button"
                    onClick={() => handleSetApproval(item.id, "approved")}
                    className={`flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl font-bold text-xs tracking-wide transition-all duration-150 border ${isApproved
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl font-bold text-xs tracking-wide transition-all duration-150 border ${isRejected
          ...
          <button
            ...
            className="w-full py-3.5 sm:py-4 rounded-xl sm:rounded-2xl bg-gradient-to-r from-blue-600 to-fuchsia-600 disabled:opacity-50 transition-all font-extrabold text-xs uppercase tracking-widest text-white shadow-lg flex items-center justify-center gap-2 active:scale-[0.99]"
          >
            {isSubmitting ? (
